...
| `loader` | A loader / sampler, and with it the `optimisation.batch_size` and `labelled_unlabelled_ratio` bindings | a recipe needs a fixed labelled/unlabelled quota per batch rather than whatever the data gives. The gradient executor takes an iterable of batches; a stage field for either key would be a card key nothing could check, which §7.1 rejects for provenance and §9.1 for hyperparameters. A design for the capability is drafted in `docs/proposals/loader.md` and is unreviewed; nothing in it is built | `fixmatch` §5.4; `scarf` §5.6; `tarnet` §5.5 |
