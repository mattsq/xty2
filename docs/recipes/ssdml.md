...
| 4 | Use deterministic ridge linear/logistic nuisances rather than the old source's random forests and `DoubleML` dependency. | The array artifact must be portable tensor state, and numpy is already a core dependency. The fixed DGP is correctly specified by these learners. | Lower capacity; intentionally favourable on the fixed linear benchmark; not a general-purpose learner comparison. |
...
| Nuisance learner family | Ridge linear outcome models and ridge logistic propensity, penalty `0.001`, intercept exempt. | Deterministic numpy implementation, portable tensor state and correct specification on section 6. |
