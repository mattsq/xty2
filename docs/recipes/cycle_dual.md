...
| 2 | Replace in-loss, in-sample `argmax q(t|x,y)` with an immutable out-of-fold side table. | The old operation is the exact circular staged fit rejected by `DESIGN.md` section 7.2 when its labels train an outcome model. | May reduce apparent training fit; should make held-out behaviour more credible. |
| 3 | Omit `G_X(t,y)`, both explicit reconstruction cycles, adversarial discriminators and entropy minimisation. | A treatment-conditioned covariate reconstruction would require a new semantic output not shared by another v1 recipe. Adding one at Gate 2 would violate the two-consumer rule. | This is a materially smaller method; the old XTYLearner loss is not expected to be numerically reproduced. |
| 4 | Use a reused TARNet Gaussian outcome head rather than the old deterministic `G_Y` MLP. | Candidate-treatment means and likelihoods must satisfy the existing `Y_GIVEN_XT` distribution contract. | MSE and fixed-scale Gaussian NLL have the same optimum up to scale and a constant on complete rows; the architecture still differs. |
| 5 | Support categorical treatment only. | Continuous treatment is outside xty2 v1 scope. | No categorical effect for the declared benchmark; legacy continuous-treatment behaviour is absent. |
| 6 | Use all hard pseudo-labels with no entropy or confidence threshold. | P10 deliberately exposes hard argmax only. A threshold or soft-label policy waits for a reviewed card and a second consumer. | Noisy labels can bias the outcome fit; the fixed benchmark is intended to reveal that rather than tune it away. |
